...
## Architecture
- **Domain-first**: Pure functions live in `src/domain` (models, `createBoard`, `addCard`, `moveCard`, `validators`). Business rules (adjacent moves only, DOING cap = 2, DONE is terminal) are enforced here and unit-tested.
...
## Testing strategy
- **Unit** (`src/tests/domain`): High-coverage tests for `moveCard`, repository load/save, error paths, and limits.
